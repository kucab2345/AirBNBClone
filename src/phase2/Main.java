...
public class Main {
		...
		try 
		{
			do 
			{
				...
				while (loginState) 
				{
					...
					if (count == 1) 
					{
						MakeReservations(input, connection);
					} 
					else if (count == 2)
					{
						MakingListing(input, connection);									
					}
					else if (count == 3)
					{
						AddAvailabilityDates(input, connection);
					} 
					else if (count == 4)
					{
						RecordingStays(input, connection);
					}
					else if (count == 5)//All types of feedback
					{
						TypesOfFeedback(input, connection);
					} 
					else if(count == 6)
					{
						TemporaryHousingBrowsing(input, connection);
					}
	...
	public static void MakeReservations(BufferedReader input, Connector connection)
	{
		try
		{
	...
	public static void MakingListing(BufferedReader input, Connector connection)
	{
	...
	public static void AddAvailabilityDates(BufferedReader input, Connector connection)
	{
		try
		{
			Reserve reservation = new Reserve(login, password);
			System.out.println("Would you like to remove availability dates or add more?");
			System.out.println("1. Remove Dates: ");
			System.out.println("2. Add More Dates: ");
			int availabilityChoice = 0;
			String requested;
			do{
				requested = input.readLine();
			}while(!requested.equals("1") && !requested.equals("2"));
			
			availabilityChoice = Integer.parseInt(requested);
			if(availabilityChoice == 1)
			{
				...
				{
					HashSet<String> housesAvail = reservation.DisplayTempHousesAvailable(login, connection.stmt, false);
					System.out.println("Please select which housing you would like to remove dates from. ");
					do
					{
						System.out.println("Enter a valid temporary housing ID number:");
						requested = input.readLine();	
					...
					System.out.println("Which period of dates would you like to remove?");
					Available avail = new Available();
					int thidToRemove = Integer.parseInt(requested);
					avail.THAvailabilityPeriods(thidToRemove, connection.stmt);
					System.out.println("Please type only the period ID integer associated with the dates you want to remove: ");
					requested = input.readLine();
					int periodIDToRemove = Integer.parseInt(requested);
					if(avail.RemoveDate(thidToRemove, periodIDToRemove, connection.stmt))
					{
						System.out.println("Successfully removed dates from Temporary House ID: " + thidToRemove + ".");
					}
					else
					{
						System.out.println("Failed to remove the date from this temporary housing's availability.");
					}
					System.out.println("Would you like to remove another? (yes/no)");
					answer = input.readLine();
					answer = answer.toLowerCase();
				}while(answer.equals("yes") || answer.equals("y") || answer.equals("yee"));
	...
	public static void RecordingStays(BufferedReader input, Connector connection)
	{
		try
		{
	...
	public static void TypesOfFeedback(BufferedReader input, Connector connection)
	{
		try
		{
			int feedbackCount = 0;
			...
			FeedbackDesired(feedbackCount, input, connection);
		}
		catch(Exception e)
		{
			System.err.println(e.getMessage() + "\n" + e.getStackTrace());
	...
	public static void FeedbackDesired(int feedbackCount, BufferedReader input, Connector connection)
	{
		try
		{
	...
	public static void TemporaryHousingBrowsing(BufferedReader input, Connector connection)
	{
		try
		{
